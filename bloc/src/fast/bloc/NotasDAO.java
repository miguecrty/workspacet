...
public class NotasDAO {
	...
	public Nota obtener(int id) throws DAOException {
		...
		ejemplo.setTexto("Esta es una nota de ejemplo.");
		ejemplo.setCategoria("Esta es una categoria de ejemplo.");
		ejemplo.setColor("Esta es una nota de ejemplo.");
	...
	public Nota obtener(int id, String usuario) throws DAOException {
		...
		try {
			...
			if (rs.next()) {
				...
				nota.setCategoria(rs.getString(4));
				nota.setColor(rs.getString(5));
				nota.setTexto(rs.getString(6));
				nota.setUrlimagen(rs.getString(7));
	 ...
	 * Los campos que ya tienen datos son: nombre_usuario, titulo, texto, urlimagen
	...
	public boolean insertar(Nota nota) throws DAOException {
		...
		try {
			conn = ds.getConnection();
			String sql = "INSERT INTO notas (nombre_usuario, titulo, texto, categoria , color , urlimagen) VALUES (?,?,?,?,?,?)";
			PreparedStatement st = conn.prepareStatement(sql);
			st.setString(1, nota.getNombreUsuario());
			st.setString(2, nota.getTitulo());
			st.setString(3, nota.getCategoria());
			st.setString(4, nota.getColor());
			st.setString(5, nota.getTexto());
			st.setString(6, nota.getUrlimagen());
